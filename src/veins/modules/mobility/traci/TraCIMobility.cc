...
simtime_t TraCIMobility::getAccidentTime()
{
    simtime_t accidentStart = par("accidentStart");
    return accidentStart;
}

simtime_t TraCIMobility::getAccidentDuration()
{
    simtime_t accidentDuration = par("accidentDuration");
    return accidentDuration;
}

int TraCIMobility::getAccidentCount()
{
...
void TraCIMobility::handleSelfMsg(cMessage *msg)
{
	if (msg == startAccidentMsg) {
	    getVehicleCommandInterface()->setSpeedMode(7);
		getVehicleCommandInterface()->setSpeed(getVehicleCommandInterface()->getMaxSpeed());
		simtime_t accidentDuration = par("accidentDuration");
		scheduleAt(simTime() + accidentDuration, stopAccidentMsg);
		accidentCount--;
	}
	else if (msg == stopAccidentMsg) {
	    getVehicleCommandInterface()->setSpeedMode(31);
